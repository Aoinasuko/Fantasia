...
FINAL_DESTINATION_NAME_FIELDS = (
    "final_destination_name",
    "fixed_final_destination_name",
    "final_location_name",
    "final_goal_name",
    "goal_location_name",
    "last_dungeon_name",
...
def generate_location_from_template(
    ...
    *,
    ...
    grid_y: int = 0,
    rng: random.Random | None = None,
    source: str = "location_generation",
    discovered: bool = False,
    fixed_name: str = "",
    explicit_name: str = "",
    branch_anchor: str = "",
    description_context: str = "",
    boss_required: bool | None = None,
) -> LocationData:
    generator = TemplateWorldGenerator(
        engine,
        world,
    ...
    if rng is not None:
        generator.rng = rng
    spec = generator.generate_location(
        slot_id=slot_id or f"loc_dynamic_{len(world.locations):03d}",
        category=category,
        type_hint=type_hint,
        role=role,
        index=index,
    ...
    if location is None:
        raise RuntimeError("location generation did not create a location")
    return location
...
class TemplateWorldGenerator:
    ...
    def _build_main_route(self) -> list[dict[str, Any]]:
        ...
        final_destination_fixed_name = _final_destination_fixed_name_from_theme(self.theme, self.world, self.premise)
        if final_destination_fixed_name:
            self.world.extra["fixed_final_destination_name"] = final_destination_fixed_name
        ...
        self.world.extra["local_world_skeleton"] = {
            ...
            "fixed_final_destination_name": final_destination_fixed_name,
        }
        ...
        for index, route_slot in enumerate(ROUTE_SLOTS):
            ...
            fixed_name = final_destination_fixed_name if role == "final_destination" else ""
            crime_risk_multiplier = _template_crime_risk_multiplier(template) if category == "settlement" else None
            location_name = self._location_name_for_template(
                str(template.get("name") or self._fallback_template_name(category, template_type, index)),
                fixed_name=fixed_name,
            )
            ...
            if crime_risk_multiplier is not None:
                location.extra["crime_risk_multiplier"] = crime_risk_multiplier
                location.flags["crime_risk_multiplier"] = crime_risk_multiplier
                payload = location.extra.get("world_generation_payload")
                if isinstance(payload, dict):
                    payload["crime_risk"] = crime_risk_multiplier
        ...
        self._complete_generated_location_specs(specs, source="template_world_generation")
        return specs
    ...
    def generate_location(
        ...
        *,
        ...
        category = "highway" if category == "single" else str(category or "").strip()
        role = str(role or "").strip()
        template_type = str(type_hint or "").strip()
        if category == "dungeon" and template_type in {"", "dungeon", "final_destination"}:
            template_type = self._choose_dungeon_type(
                " ".join(
                    str(part or "")
                    for part in (
        ...
        template_type = str(template.get("type") or template_type or category)
        if danger is None:
            danger_value = self.engine._local_world_danger_for_distance(index, self.rng, seed=f"{self.world.world_name}:{slot_id}")
            if role == "starting_settlement":
                danger_value = 0
            if role == "final_destination":
                danger_value = max(danger_value, self.rng.randint(WORLD_FINAL_DANGER_MIN, WORLD_FINAL_DANGER_MAX))
        else:
            danger_value = _safe_int(danger, 0)
        if role == "starting_settlement":
            danger_value = 0
        if role == "final_destination":
            danger_value = max(danger_value, self.rng.randint(WORLD_FINAL_DANGER_MIN, WORLD_FINAL_DANGER_MAX))
        danger_value = _clamp_world_danger(danger_value)
        fixed_name = _clean_fixed_location_name(fixed_name)
        crime_risk_multiplier = _template_crime_risk_multiplier(template) if category == "settlement" else None
        location_name = self._location_name_for_template(
            explicit_name or str(template.get("name") or self._fallback_template_name(category, template_type, index)),
            fixed_name=fixed_name,
        )
        location = self.world.ensure_location(location_name, str(template.get("desc") or ""))
        location.area = f"branch:{branch_anchor}" if branch_anchor else f"route:{index}"
        location.extra.update(
            {
                "slot_id": slot_id,
                "main_node_type": category,
                ...
                "role": role,
                "location_kind": self._location_kind(category, template_type),
                "danger_level": danger_value,
                "danger_source": source,
                "grid_x": grid_x,
                "grid_y": grid_y,
                "grid_distance": index,
                "world_template": deepcopy(template),
        ...
        if crime_risk_multiplier is not None:
            ...
            if isinstance(payload, dict):
                payload["crime_risk"] = crime_risk_multiplier
        if branch_anchor:
            location.extra["branch_anchor_location"] = branch_anchor
        location.flags["discovered"] = bool(discovered)
        if category == "settlement":
            location.flags["settlement"] = True
            self._install_settlement_templates(location, template_type, self.rng)
        elif category == "dungeon":
            ...
            location.flags["final_destination"] = role == "final_destination"
            location.extra["final_destination"] = role == "final_destination"
            location.extra["boss_required"] = bool(boss_required if boss_required is not None else role in {"final_destination", "quest_dungeon", "tool_generated_dungeon"})
            self.install_dungeon_subnode_graph(location, self.rng, source=source)
        else:
            self._install_highway_template(location, template_type, self.rng)
        spec = {
            "slot_id": slot_id,
        ...
        self._complete_generated_location_specs([spec], source=source)
        location = self.world.locations.get(str(spec.get("name") or ""), location)
        self.engine._set_location_graph_node(self.world, location.name, kind=str(spec.get("kind") or ""), danger=danger_value, location=location)
        return spec

    def _complete_generated_location_specs(self, specs: list[dict[str, Any]], *, source: str) -> None:
        for spec in specs:
            ...
            self._describe_generated_location(
                location,
                spec,
                template,
                branch_anchor=str(spec.get("branch_anchor") or location.extra.get("branch_anchor_location") or ""),
                description_context=str(spec.get("description_context") or ""),
            )
            location = self.world.locations.get(str(spec.get("name") or ""), location)
            if spec.get("category") == "settlement":
                self._sync_facility_locations(location)
                self._describe_generated_facilities(location, spec)
                self._apply_facility_owner_names([spec])
            elif spec.get("category") == "dungeon":
                self._describe_generated_internal_nodes(location, spec, template)
                self._ensure_generated_location_boss(location, source)
            self.engine._set_location_graph_node(
                self.world,
                location.name,
                kind=str(spec.get("kind") or ""),
                danger=_safe_int(spec.get("danger"), 0),
                location=location,
            )

    def _describe_generated_location(
        self,
        location: LocationData,
        spec: dict[str, Any],
        ...
        *,
        ...
        fixed_name = _clean_fixed_location_name(spec.get("fixed_name") or location.extra.get("fixed_name"))
        prompt = {
            "world": {
                "world_name": self.world.world_name,
                "overview": _short_text(self.world.overview, 900),
                "structure_description": _short_text(self.world.structure_description, 600),
                "final_destination_concept": str(self.theme.get("final_destination_concept") or ""),
            },
            "premise": _short_text(self.premise, 1200),
            "existing_location_names": [name for name in self.world.locations if name != location.name],
            "target_location": {
                ...
                "fixed_name": fixed_name,
                "branch_anchor": branch_anchor,
                "description_context": _short_text(description_context, 500),
            },
            "template": {
            ...
            "rules": [
                "Return one JSON object under location with name and description.",
                "If target_location.fixed_name is set, return exactly that value as location.name and only generate the description.",
                "Use the template only as genre guidance. Do not copy template.name or template.desc verbatim.",
                "Do not change category, type, role, danger, graph, items, NPCs, or connections.",
                "Do not mention slot_id, category, type, template, target_type, danger rule, placeholder, unnamed, or internal data.",
                "Write Japanese in-world prose.",
            ],
        }
        try:
            response = self.engine._chat_json(
                "location_generate_location_describer",
                [
                    {
                        "role": "system",
                        "content": "Name and describe exactly one Fantasia location. Return compact JSON only.",
                    },
            ...
            self.world.extra.setdefault("location_generation_errors", []).append({"slot_id": spec.get("slot_id"), "stage": "location_description", "error": str(exc)})
            return
        item = response.get("location") if isinstance(response, dict) and isinstance(response.get("location"), dict) else None
        if item is None:
            items = _response_items(response)
            item = items[0] if items else {}
        if not isinstance(item, dict):
            return
        item = {**item, "slot_id": spec.get("slot_id")}
        if fixed_name:
            item["name"] = fixed_name
            item["title"] = fixed_name
        self._apply_location_description({str(spec.get("slot_id") or ""): spec}, str(spec.get("slot_id") or ""), item)
        self.world.history.append({"manager": "location_generate_location_describer", "slot_id": spec.get("slot_id"), "response": _strip_response_metadata(response)})

    def _describe_generated_facilities(self, location: LocationData, spec: dict[str, Any]) -> None:
        slots = [
            slot
            for slot in self._description_slots([spec])
            if isinstance(slot, dict) and str(slot.get("target") or "") == "facility"
        ]
        if not slots:
            return
        prompt = {
            "world": {
                "world_name": self.world.world_name,
                "overview": _short_text(self.world.overview, 700),
            },
            "location": {"name": location.name, "description": _short_text(location.description, 360)},
            "facility_slots": slots,
            "rules": [
                "Return one item per slot_id with name and description only.",
                "Use template_name and template_desc only as facility genre guidance; do not copy them verbatim.",
                "For shop facilities, include npc_name naturally in the facility name when possible.",
                "Do not describe NPC appearance or personality here.",
                "Write Japanese in-world prose.",
            ],
        }
        try:
            response = self.engine._chat_json(
                "location_generate_facility_describer",
                [
                    {"role": "system", "content": "Name and describe settlement facilities. Return compact JSON only."},
                    {"role": "user", "content": _ai_json(prompt)},
                ],
                max_tokens=max(700, min(1800, 300 + len(slots) * 160)),
                world_name=self.world.world_name,
                player_name=self.player_name,
                retries=1,
            )
        except Exception as exc:
            self.world.extra.setdefault("location_generation_errors", []).append({"slot_id": spec.get("slot_id"), "stage": "facility_description", "error": str(exc)})
            return
        self._apply_description_response([spec], response)
        self.world.history.append({"manager": "location_generate_facility_describer", "slot_id": spec.get("slot_id"), "response": _strip_response_metadata(response)})

    def _describe_generated_internal_nodes(self, location: LocationData, spec: dict[str, Any], template: dict[str, Any]) -> None:
        ...
        for node_id, node in list(nodes.items()):
            if not isinstance(node, dict):
                continue
            slot_id = f"{spec.get('slot_id')}:subnode:{node_id}"
            prompt = {
                "world": {
                    "world_name": self.world.world_name,
                    "overview": _short_text(self.world.overview, 700),
                ...
                "location": {
                    "name": location.name,
                    "description": _short_text(location.description, 360),
                    "type": spec.get("type"),
                    "role": spec.get("role"),
                    "danger": spec.get("danger"),
                },
                ...
                "subnode_template": {
                    "id": node_id,
                    "kind": node.get("kind"),
                    "name": node.get("template_name") or node.get("name"),
                    "desc": node.get("template_desc") or node.get("description"),
                },
                "rules": [
                    "Return one JSON object with name and description.",
                    "Use the location and subnode templates as genre guidance. Do not copy template text verbatim.",
                    "Do not change graph, item tables, enemy rates, boss flags, or mechanics.",
                    "Do not mention internal IDs, template, target_type, item_tabel, generate_boss, or danger rule.",
                    "Write Japanese in-world prose.",
                ],
            }
            try:
                response = self.engine._chat_json(
                    "location_generate_subnode_describer",
                    [
                        {"role": "system", "content": "Name and describe exactly one internal subnode. Return compact JSON only."},
                        {"role": "user", "content": _ai_json(prompt)},
                    ],
                    max_tokens=360,
                    world_name=self.world.world_name,
                ...
                self.world.extra.setdefault("location_generation_errors", []).append({"slot_id": slot_id, "stage": "subnode_description", "error": str(exc)})
                continue
            item = response.get("subnode") if isinstance(response, dict) and isinstance(response.get("subnode"), dict) else None
            if item is None:
                item = response.get("location") if isinstance(response, dict) and isinstance(response.get("location"), dict) else None
            if item is None:
                items = _response_items(response)
            ...
            if isinstance(item, dict):
                self._apply_subnode_description({str(spec.get("slot_id") or ""): spec}, slot_id, {**item, "slot_id": slot_id})
            self.world.history.append({"manager": "location_generate_subnode_describer", "slot_id": slot_id, "response": _strip_response_metadata(response)})

    def _sync_facility_locations(self, location: LocationData) -> None:
        facilities = location.extra.get("facilities")
        if not isinstance(facilities, list):
            return
        for facility in facilities:
            if isinstance(facility, dict):
                facility["location_name"] = location.name

    def _ensure_generated_location_boss(self, location: LocationData, source: str) -> None:
        if not location.flags.get("dungeon") and str(location.extra.get("location_kind") or "") != "dungeon":
            return
        graph = location.extra.get(SUBNODE_GRAPH_KEY)
        nodes = graph.get("nodes", {}) if isinstance(graph, dict) and isinstance(graph.get("nodes"), dict) else {}
        node_requires_boss = any(isinstance(node, dict) and _as_bool(node.get("generate_boss")) for node in nodes.values())
        boss_required = node_requires_boss or _as_bool(location.extra.get("boss_required")) or _as_bool(location.extra.get("final_destination"))
        if not boss_required:
            return
        payload = location.extra.setdefault("world_generation_payload", {})
        if isinstance(payload, dict):
            payload["boss_required"] = True
            payload.setdefault("location", location.name)
            payload.setdefault("narration", location.description)
        if getattr(self.engine.state, "world_data", None) is self.world and hasattr(self.engine, "_ensure_generated_dungeon_boss"):
            event = self.engine._ensure_generated_dungeon_boss(
                location.name,
                source,
                {"boss_required": True, "location": location.name, "narration": location.description},
            )
            if event:
                return
        if hasattr(self.engine, "_ensure_world_generation_dungeon_boss"):
            old_player_name = getattr(self.engine.state, "player_name", "")
            if self.player_name:
                self.engine.state.player_name = self.player_name
            try:
                event = self.engine._ensure_world_generation_dungeon_boss(self.world, location.name, self.premise or source)
                if event:
                    return
            except Exception as exc:
                self.world.extra.setdefault("location_generation_errors", []).append(
                    {"location": location.name, "stage": "boss_generation", "error": str(exc)}
                )
            finally:
                if old_player_name:
                    self.engine.state.player_name = old_player_name
        if hasattr(self.engine, "_ensure_generated_dungeon_boss"):
            old_world = getattr(self.engine.state, "world_data", None)
            try:
                self.engine.state.world_data = self.world
                self.engine._ensure_generated_dungeon_boss(
                    location.name,
                    source,
                    {"boss_required": True, "location": location.name, "narration": location.description},
                )
                ...
                if old_world is not None:
                    self.engine.state.world_data = old_world
    ...
    def _seed_template_subnode_loot(self, location: LocationData, graph: dict[str, Any], *, source: str) -> None:
        ...
        for node_id, node in nodes.items():
            ...
            initial_loot_store[str(node_id)] = deepcopy(slot)

    def _describe_template_slots(self, specs: list[dict[str, Any]]) -> None:
        self._complete_generated_location_specs(specs, source="template_world_legacy_describe")
    ...
    def _apply_location_description(self, spec_by_id: dict[str, dict[str, Any]], slot_id: str, item: dict[str, Any]) -> None:
        ...
        if not location:
            return
        fixed_name = _clean_fixed_location_name(spec.get("fixed_name") or location.extra.get("fixed_name"))
        new_name = fixed_name or _clean_llm_name(item.get("name") or item.get("title") or "", fallback=old_name)
        ...
        if fixed_name:
            location.extra["fixed_name"] = fixed_name
            location.extra["fixed_final_destination_name"] = fixed_name
    ...
    def _location_name_for_template(self, base_name: str, *, fixed_name: str = "") -> str:
        fixed = _clean_fixed_location_name(fixed_name)
        if fixed:
            return fixed
        return self._unique_location_name(base_name)
...
def _final_destination_fixed_name_from_theme(theme: dict[str, Any], world: WorldData | None = None, premise: str = "") -> str:
    raw_theme = world.extra.get("raw_create_world_theme") if world is not None and isinstance(world.extra, dict) else {}
    raw_theme = raw_theme if isinstance(raw_theme, dict) else {}
    for source in (theme, raw_theme):
        if not isinstance(source, dict):
            continue
        for field in FINAL_DESTINATION_NAME_FIELDS:
            candidate = _clean_fixed_location_name(source.get(field))
            if candidate:
                return candidate
    for text in (
        str(theme.get("final_destination_concept") or ""),
        str(raw_theme.get("final_destination_concept") or ""),
        str(premise or ""),
    ):
        candidate = _extract_fixed_location_name_from_text(text)
        if candidate:
            return candidate
    return ""


def _extract_fixed_location_name_from_text(value: Any) -> str:
    ...
    for opener, closer in quote_pairs:
        ...
        if start >= 0 and end > start:
            candidate = _clean_fixed_location_name(text[start + len(opener) : end])
            if candidate:
                return candidate
    markers = (
        "\u6700\u7d42\u76ee\u7684\u5730",
    ...
    pattern = rf"(?:{marker_pattern})\s*(?:\u306f|\u3092|\u3068\u3057\u3066|[:\uff1a])?\s*([^。\u3001\u300c\u300d\u300e\u300f\r\n]{{2,36}})"
    match = re.search(pattern, text)
    if match:
        candidate = _clean_fixed_location_name(match.group(1))
        if candidate:
            return candidate
    return ""


def _clean_fixed_location_name(value: Any) -> str:
    text = re.sub(r"\s+", " ", str(value or "").strip())
    text = text.strip(" \t\r\n\u3000:：,，.。")
    text = text.strip("\"'\u300c\u300d\u300e\u300f")
    if not text:
    ...
    if len(text) < 2 or len(text) > 36:
        return ""
    lowered = text.casefold()
    if any(marker in text for marker in ("\u672a\u547d\u540d", "\u4eee\u540d", "\u4eee\u79f0", "\u521d\u671f\u5730\u70b9")):
        return ""
    if any(marker in lowered for marker in ("unnamed", "placeholder", "todo", "tbd", "slot_id", "template")):
        return ""
    generic_names = {
        "final destination",
        "final_destination",
    ...
    if lowered in generic_names or text in generic_names:
        return ""
...
def _template_crime_risk_multiplier(template: dict[str, Any]) -> float | None:
    if not isinstance(template, dict) or template.get("crime_risk") in (None, ""):
        return None
    return max(0.0, min(2.0, _safe_float(template.get("crime_risk"), 1.0)))
