...
class GameEngine:
    ...
    def create_world(
        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "world_name, overview, structure_description, structure を持つJSONだけを返してください。"
        ...
        self._enrich_initial_characters(player, premise_text, world)
        ...
        self._set_world_time_total_hours(0)
    ...
    def generate_character_setup_traits(self, character: CharacterData) -> list[dict[str, Any]]:
        response = self._create_trait(character.name or self.state.player_name, self.state.world_data, character)
        self._apply_character_traits(character, response)
        return character.traits

    def generate_character_setup_skills(self, character: CharacterData) -> list[dict[str, Any]]:
        response = self._create_skill(character.name or self.state.player_name, self.state.world_data, character)
    ...
    def toggle_player_equipment(self, inventory_index: int, *, save_game: bool = True) -> dict[str, Any]:
        ...
        if item.get("equipped"):
            event = self._unequip_player_slot(str(item.get("equipment_slot") or equipment_slot_for_category(str(item.get("category") or ""))), source="inventory")
        else:
    ...
    def _remove_player_item_reference(self, value: Any, *, source: str, reason: str = "") -> dict[str, Any] | None:
        ...
        if item.get("equipped"):
            self._unequip_player_slot(str(item.get("equipment_slot") or ""), source=source, reason=reason)
        quantity = 1
    ...
    def _ensure_settlement_facilities(self, settlement: LocationData) -> list[dict[str, Any]]:
        ...
        for item in facilities:
            ...
            record = {
                ...
                "location_name": str(item.get("location_name") or f"{settlement.name} / {name}"),
    ...
    def _set_character_presence(self, character: CharacterData, location: str, state: str = "present") -> None:
        ...
        character.state = state or character.state or "present"
        ...
        messages = [
            ...
            },
            {
                ...
                "content": (
                    ...
                    "Prefer ordinary settlement facilities such as guilds, shops, inns, temples, clinics, libraries, stables, and markets when plausible."
                ),
            },
        ]
        return self._chat_json(
    ...
    def _facility_from_response(self, response: dict[str, Any], requested: str, settlement: LocationData) -> dict[str, Any]:
        ...
        return {
            ...
            "location_name": str(raw.get("location_name") or f"{settlement.name} / {name}"),
    ...
    def _enrich_initial_characters(self, player_name: str, premise: str, world: WorldData) -> None:
    ...
    def _create_trait(self, player_name: str, world: WorldData, character: CharacterData) -> dict[str, Any]:
        world_payload = _ai_json(
            _world_ai_context(
                world,
                include_locations=False,
    ...
    def _create_skill(self, player_name: str, world: WorldData, character: CharacterData) -> dict[str, Any]:
        world_payload = _ai_json(
            _world_ai_context(
                world,
                include_locations=False,
        ...
        messages = [
            ...
            },
            {
                ...
                "content": (
                    ...
                    f"キャラクターデータ: {character_payload}\n"
                    f"{power_instruction}\n"
                    "このキャラクターのスキル、効果、種別、SPコスト、有用性を生成してください。"
                ),
            },
        ]
        return self._chat_json(
    ...
    def _player_level(self, character: CharacterData | None = None) -> int:
        ...
        if player:
            candidates.extend([player.extra.get("level"), player.flags.get("level")])
        if self.state.party and isinstance(self.state.party[0], dict):
    ...
    def _resolve_active_quest_action(
        ...
        if referee.get("event"):
            event_resolution = self._quest_referee_event_resolve(action, quest, referee)

        narration_parts = [str(referee.get("narration") or referee.get("text") or "")]
        if event_resolution:
            narration_parts.append(str(event_resolution.get("narration") or event_resolution.get("text") or ""))
...
def _facility_record(name: str, settlement_name: str, facility_type: str = "") -> dict[str, Any]:
    ...
    return {
        ...
        "location_name": f"{settlement_name} / {name}",
        "source": "create_settlement_detail",
    }


def _facility_exists(facilities: list[dict[str, Any]], name: str) -> bool:
    return any(_facility_name_matches(str(item.get("name") or ""), name) for item in facilities)
